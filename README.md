...
The Forge interfaces follow this project's `1.21.1-neoforge` layouts, implemented with native Minecraft/AE2 containers. All six screen types and JEI pages share square frames, cool-grey surfaces, shadowless text and dark text fields; LDLib2 is not required.

- Matter deconstruction and reproduction highlight their buttons only while the corresponding operation is enabled. Stopped operations use the normal button style; an enabled operation may still be waiting for materials, power or network access.
- Auto Crafting has white configuration gears and enable boxes centered below their pattern slots. Tabs use their button highlight without an extra underline.
