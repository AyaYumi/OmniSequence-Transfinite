...
万物演算核心使用 `SAFE` 聚合模式加速确定性配方树。物品替代样板会从该规划器中保守回退；仅流体替代仍具确定性，可继续使用快速路径。规划回退不会阻止兼容的运行时批量发配：物品替代样板永久允许进入批量路径，实际输入仍由 AE2 原生逻辑选择。容器返还、动态输入、循环或未知样板行为也会自动回退到 AE2 原生计算，避免为了速度牺牲正确性。
...
装配矩阵构序重写核心与构序阵列会以持久执行模型处理可复用输入。同键返还物品（包括标记为不可损坏的物品）可在整个批次中重复使用；有限耐久工具仅在每次合成都确定增加恰好 1 点损伤时才会批量执行。带耐久附魔以及其他概率性或依赖上下文的状态变化会回退 AE2 原生逐份路径，水桶变为空桶等换键返还也继续逐份处理。

供应器接受批次后会跨存档、区块卸载和服务器重启持有完整执行状态。取消 AE2 合成任务会持久停止剩余执行，并精确退回尚未消耗的材料以及可复用物品的当前状态；已经完成的产物仍然有效，重载后也不会恢复已取消任务。批量扩展会对实际合并后的输入调用 AE2 原生样板能耗计算，保持原版合成能耗语义。
