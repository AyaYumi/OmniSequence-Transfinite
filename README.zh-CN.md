...
当前已知限制：产物相同、可替代原料范围重叠时，队列拆分可能改用另一条配方的耗时与能耗；重载时新增更高优先级的重叠配方，也可能使已有队列保持等待。这两类情况与产物不同的配方隔离是不同问题，2.0.5 尚未修复。
...
使用整合包实际 JEI 与 LDLib2 JAR 进行坐标回归的步骤见[界面测试说明](tools/ui/README.md)，不需要打开游戏。
