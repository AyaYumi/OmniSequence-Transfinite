...
和 [物质研究／KubeJS API](docs/matter-research-api.zh-CN.md)。AELIS 规划接口由独立前置提供。
...
- 提供分子构序重写阵列、装配矩阵构序重写核心、万物演算核心、超限算枢，以及由构序阵列控制器管理的构序阵列多方块。
- 支持大型结构投影、一键搭建、一键拆卸、跨区块暂停恢复和动态视觉效果。
- 物质构筑井采用占地 41×41、高 27 格的白金创生舱，配套原生 16×16 珍珠白、浅银灰与香槟金动画材质。服务方块可安装在正前方 24 格和中央环段 20 格，原外围九个位置已停用。
...
- 三种大型多方块占用区块的整个高度禁止自然生成生物，包括怪物、动物、水生生物和蝙蝠。保护随成型结构及施工／拆卸范围生效，刷怪笼、刷怪蛋和指令生成保留，不会清除已有生物。
- 提供可由整合包配置的物质分解、序列储存和蓝图复制系统。
- 提供主要机器、物质构筑井、研究、输入输出口和样板总成的中英文 GuideME 指南，支持实时构筑井配方与研究解锁标注。

## 自动合成与材料发配

成型的万物演算核心或在线超限算枢通过 AppliedEnhancements 的公开 `AelisCraftingPlanner` API 启用该网络的规划加速，使用前置配置的节点和时间预算。缺少可用 Omni CPU 时保留原生路径；若前置全局自动 AELIS 已开启，则由前置接管，避免重复规划。循环计划的求解、执行顺序和保种由前置处理，本模组继续负责自己的机器批量发配和虚拟 CPU 管理。
...
- 支持投影、自动施工、自动拆卸，并禁止占地区块内所有类别生物自然生成。

### 超限算枢

- 单方块 Omni 合成 CPU，六面可接线，提供独立虚拟 CPU 通道及 `Long.MAX_VALUE` 级逻辑合成存储。
- 需要已供电的 ME 网络和一个频道；默认待机消耗 16,384 AE/t。
- 相邻算枢保持独立 CPU；可恢复任务与内部材料随掉落方块保留，重新放置并联网后恢复。
- 使用 AE2 终端管理任务，没有独立机器界面、量子槽、样板库或自动区块加载。
- 安装 Advanced AE 后，默认在物质构筑井完成万物演算研究解锁，再由构筑井加工制作。

### 构序阵列控制器（构序阵列多方块）

- 默认 200 页 / 7,200 个样板槽，最多可配置 300 页 / 10,800 槽。成型样板库由 14 颗量子水晶分摊保存；搬迁时需保留带样板水晶，自动施工会优先使用它们。
- 可在控制器可接触面贴 ME 存储总线，存取全部已配置页的主样板库；每槽一个，只接收合法编码合成、锻造及切石样板。材料、量子、升级和九个被动合成槽不对外开放。新的未成型控制器可以预装样板；施工、拆卸和恢复期间暂停外部搬运。
...
- 将已配置的样板页暴露为多个逻辑样板访问终端容器，合并样板库仍由控制器统一管理。
...
响应式机器界面向 JEI 提供实际显示边界及物品命中区域；2.0.3 同时向 JEI 前景／背景绘制传递原始鼠标坐标，修复点击正确但悬浮高亮和提示框偏移的问题。
...
| `transfinite_compute_nexus` | 单方块超限算枢待机功耗 |

客户端配置分为 `tooltips` 和 `visual`。

| 配置项 | 默认值 | 作用 |
| --- | ---: | --- |
| `sequence_array.pattern_pages` | 200 | 构序阵列控制器样板页数，每页 36 槽，范围 1～300；成型后由 14 颗量子水晶分摊保存 |
| `sequence_array.build_blocks_per_tick` | 32 | 自动搭建或拆卸每 Tick 处理方块数 |
| `sequence_array.idle_power` | 128 | 构序阵列控制器待机功耗，单位 AE/t |
| `transfinite_compute_nexus.idle_power` | 16384 | 超限算枢待机功耗，单位 AE/t，需要一个已供电 ME 频道 |
...
从 2.0.2 升级前，请将第 300 页之后的样板迁出；原 1,000 页上限不再支持。
进行水晶存储迁移前保留世界备份。原构筑井外围九处服务方块需迁到正前方或中央环段，
原位置按投影补回对应结构方块。
