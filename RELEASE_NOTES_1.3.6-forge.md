# OmniSequence: Transfinite 1.3.6-forge Release Notes
...
Version 1.3.6-forge adds in-game AE2 GuideME documentation for the Forge build's three registered primary
...
3. Put `omnisequence-transfinite-1.3.6-forge.jar` in both client and server `mods`
   directories.
4. Keep exactly one active OmniSequence JAR in each `mods` directory.
5. Start the game and verify the loaded mod version is `1.3.6-forge`, then smoke-test
...
SHA-256: `8F1BE4A169D2642106DB4FA4C73C37C192D6D72E9D536363C454E54CF53B634A`

---

# 万象构序：超限 1.3.6-forge 更新说明
...
1.3.6-forge 为 Forge 构建中仍注册的三个主要方块补充了 AE2 GuideME 游戏内文档，并改进了构序阵列控制器的
操作安全性和样板管理流程。
...
3. 将 `omnisequence-transfinite-1.3.6-forge.jar` 放入客户端和服务端的 `mods` 目录。
4. 确保每个 `mods` 目录中只有一个启用中的万象构序 JAR。
5. 启动游戏并确认加载的模组版本为 `1.3.6-forge`，然后烟测按 `G` 打开 GuideME、
   Shift 快捷放入样板和拆卸二次确认。
