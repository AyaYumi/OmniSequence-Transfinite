# OmniSequence: Transfinite 1.3.7-forge-fix Release Notes

Release date: 2026-07-30

Target environment: Minecraft 1.20.1 / Forge 47.4.10+ / official Applied Energistics 2 15.4.10 or AE2-UELM 15.5.0

## Highlights

Version 1.3.7-forge-fix adds effect-cleared layouts for both fixed multiblock
structures without invalidating their legacy layouts. Their large effects
remain anchored at the same positions, and each controller lets the player
decide whether a complete legacy structure should be updated.
...
- Crafting patterns with item substitution enabled remain a conservative
  fallback from MAX_FAST planning, but are now permanently eligible for
  compatible runtime batch dispatch. The removed
  `omni_batch_allow_substitution_patterns` option is migrated out of existing
...
- The Assembler Matrix Sequence Rewrite Core and Sequence Array now own and
  persist reusable-input batches after accepting them. Saving, chunk unloading,
  and server restarts no longer lose or duplicate an in-progress batch.
- Canceling the AE2 crafting job records a persistent cancellation marker,
  stops all remaining provider-side executions, and returns the exact
  unconsumed materials together with the reusable item's current state.
  Already-completed outputs remain valid, and a canceled job cannot silently
...
- Same-key crafting remainders, including items treated as unbreakable by their
  ...
  one. Unbreaking-enchanted or otherwise probabilistic/context-dependent tools
  conservatively fall back instead of being treated as infinite.
- Key-changing remainders such as a water bucket becoming an empty bucket, plus
  ...
  path.
- Batch expansion now uses AE2's native pattern-power calculation over the
  actual combined batch input set, restoring AE2's original crafting-energy
  semantics.

## Forge Compatibility

The standalone Molecular Sequence Rewrite Array remains intentionally absent
from the Forge 1.20.1 build; its pattern and matter-rewriting functions remain
available through the Sequence Array multiblock. The Expanded AE 2.1.1 conflict
declaration from the Minecraft 1.21.1 NeoForge build is not copied because that
version and Mixin target belong to a different loader and AE2 line.

Official AE2 15.4.10 continues to use OmniSequence's long-amount compatibility
Mixins. AE2-UELM 15.5.0 is detected at startup and routes confirmation through
its native long-amount `planJob` path, avoiding incompatible `int` field
injection.

## Controller and Creative-Mode Fixes

- Both controller screens now render the vanilla translucent world backdrop
  before responsive GUI scaling, matching the inventory screen and covering
...
- Multiblock projections cache reusable 16x16x16 render sections and rebuild
  only changed sections, reducing repeated model tessellation while previewing
  large structures.
...
- Width-aware label/value rows prevent English text from overlapping in both
  ...
  notice uses normal-size wrapped text instead of shrinking the whole sentence.
...
3. Put `omnisequence-transfinite-1.3.7-forge-fix.jar` in both client and server
   `mods` directories.
...
# 万象构序：超限 1.3.7-forge-fix 更新说明

发布日期：2026-07-30

适用环境：Minecraft 1.20.1 / Forge 47.4.10+ / 官方 Applied Energistics 2 15.4.10 或 AE2-UELM 15.5.0

## 版本亮点

1.3.7-forge-fix 为两套固定多方块增加了清空特效中心的新版布局，同时保留旧版布局兼容。
...
- 开启物品替换的合成样板仍会从 MAX_FAST 规划中保守回退，但现在永久允许进入兼容的
  运行时批量发配路径。已移除的
  `omni_batch_allow_substitution_patterns` 会从现有服务端配置中迁移删除，
  且不会重置其他自定义配置；实际替换材料仍由 AE2 原生逻辑选择。
...
- 批量扩展现在按实际合并后的批次输入集合调用 AE2 原生样板能耗计算，恢复 AE2
  原版的合成能耗语义。

## Forge 兼容说明

Forge 1.20.1 构建仍不注册独立的“分子构序重写阵列”单方块；对应的样板与物质重写功能
继续由“构序阵列”多方块提供。Minecraft 1.21.1 NeoForge 构建针对 Expanded AE 2.1.1
的冲突声明也不会照搬，因为该版本及其 Mixin 目标属于不同的加载器和 AE2 版本线。

官方 AE2 15.4.10 继续使用万象构序提供的长整数下单兼容 Mixin。启动时检测到
AE2-UELM 15.5.0 后会调用其原生长整数 `planJob` 确认路径，避免注入不兼容的
`int` 字段。

## 文档与界面修复

- 两个控制器界面现在会在响应式缩放前绘制原版黑色半透明世界背景，与物品栏一致，
  并覆盖整个屏幕。
...
3. 将 `omnisequence-transfinite-1.3.7-forge-fix.jar` 放入客户端和服务端的 `mods` 目录。
