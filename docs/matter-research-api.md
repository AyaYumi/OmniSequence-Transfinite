...
| --- | --- |
| Data recipe `molecularmanipulator:matter_research` | Define research under `data/<namespace>/recipes/`; KubeJS uses `ServerEvents.recipes` and `event.custom`. |
| `MatterResearchApi.definitions(Level)` | Returns `List<MatterResearchRecipe>`, sorted by `sort_order`, then ID; optional-mod availability is respected. |
...
| `productionProfile(controller, MatterFabricationRecipe)` | Returns `(parallel, ticks)` for current completed branches; actual batch size also depends on materials, power and per-key capacity. |
...
On Forge 1.20.1, pass `MatterResearchRecipe` / `MatterFabricationRecipe` directly,
not `RecipeHolder`. Both custom recipes provide `id()` / `getId()`; `value()`
returns the recipe itself as a source-port convenience. Create resource IDs with
`new ResourceLocation("namespace:path")`. Item data uses `nbt`, recipe directories
use `recipes/`, and optional ingredients use Forge `conditions`. Recompile Java
integrations for this branch instead of loading a NeoForge 1.21.1 integration JAR.
...
研究是普通配方类型 `molecularmanipulator:matter_research`，位于 `data/<namespace>/recipes/<path>.json`。KubeJS 使用 `ServerEvents.recipes` 的 `event.custom` 添加或替换，不需要额外适配插件。
...
修改内置研究时，先 `event.remove({id: 'molecularmanipulator:research/ae_foundation'})`，然后注册同 ID 的完整新定义。可从 JAR 的 `data/molecularmanipulator/recipes/research/` 取出原 JSON 作为模板，保留想继续解锁的 `unlocks`。保持研究 ID 即可沿用完成次数。`depths` 改成几项就有几次研究，可完全替换每轮成本与加成。

涉及可选模组物品时，顶层还须添加 `conditions`，使缺少模组时跳过 Ingredient 解析：

```json
"conditions": [{"type": "forge:mod_loaded", "modid": "advanced_ae"}]
```
...
本分支的 `definitions` 返回 `List<MatterResearchRecipe>`；`productionProfile` 和
`canUseRecipe` 直接接收 `MatterFabricationRecipe`，无需 `RecipeHolder` 包装。
下列 `existingResearch` 和 `parentResearch` 都是 `MatterResearchRecipe` 实例。
