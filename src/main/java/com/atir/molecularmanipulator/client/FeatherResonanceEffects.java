...
final class FeatherResonanceEffects {
    ...
    private static void ornament(PoseStack.Pose pose, VertexConsumer out, double phase, double radius,
        ...
        for (int spoke = 0; spoke < 4; spoke++) {
            double angle = spoke * (Math.PI * 2) / 4;
    ...
    private static void hub(PoseStack.Pose pose, VertexConsumer out, boolean glow, int primary, int secondary) {
        arc(pose, out, 0.62, DIAL_Y, 0, (Math.PI * 2), 20, glow ? 0.05F : 0.021F, primary, dialAlpha(glow, 0.56));
        arc(pose, out, 0.28, DIAL_Y, 0, (Math.PI * 2), 12, glow ? 0.05F : 0.021F, secondary, dialAlpha(glow, 0.46));
        for (int spoke = 0; spoke < 4; spoke++) {
            double angle = spoke * (Math.PI * 2) / 4 + Math.PI / 4;
    ...
    private static int dialAlpha(boolean glow, double intensity) {
        int band = (int) Math.round(com.atir.molecularmanipulator.util.MathCompat.clamp(intensity, 0, 1) * (DEPTH_BANDS.length - 1));
