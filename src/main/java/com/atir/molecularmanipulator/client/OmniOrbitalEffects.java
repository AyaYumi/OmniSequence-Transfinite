...
final class OmniOrbitalEffects {
    ...
    static void render(PoseStack stack, VertexConsumer out, double angle, float activity,
        ...
        float drive = Math.clamp(activity, 0, 1);
        float pulse = Math.clamp(completion, 0, 1);
        var pose = stack.last();
    ...
    private static void drawOrbits(PoseStack.Pose pose, VertexConsumer out, double angle,
        ...
        for (var orbit : OmniCrownGeometry.orbits()) {
            ...
            for (double offset : new double[] {-0.68, 0.68}) {
                orbitArc(pose, out, orbit, 0, Math.TAU, segments, offset,
            ...
            if (index == 0) {
                ...
                for (int group = 0; group < 24; group++) {
                    double phase = group * Math.TAU / 24;
                ...
                for (int panel = 0; panel < panels; panel++) {
                    double start = panel * Math.TAU / panels + angle * (index == 1 ? 0.0018 : -0.0018);
                    double sweep = 0.42;
            ...
            for (int packet = 0; packet < packets; packet++) {
                double head = packet * Math.TAU / packets + angle * (0.009 + index * 0.001);
    ...
    private static void drawStarMap(PoseStack.Pose pose, VertexConsumer out, double angle, float drive,
        ...
        for (int band = 0; band < latitudeCount; band++) {
            ...
            for (int step = 0; step < segments; step++) {
                ...
                line(pose, out, sphere(8.45, spin + step * Math.TAU / segments, latitude),
                        sphere(8.45, spin + (step + 1) * Math.TAU / segments, latitude),
        ...
        for (int plane = 0; plane < (detailed ? 3 : 2); plane++) {
            ...
            for (int step = 0; step < segments; step++) {
                double a = step * Math.TAU / segments, b = (step + 1) * Math.TAU / segments;
        ...
        for (int panel = 0; panel < panels; panel++) {
            double lon = panel * Math.TAU / panels + spin;
        ...
        if (pulse > 0.001F) {
            ...
            for (int step = 0; step < segments; step++) {
                line(pose, out, sphere(8.95, step * Math.TAU / segments, latitude),
                        sphere(8.95, (step + 1) * Math.TAU / segments, latitude),
    ...
    private static void drawCoordinateMarkers(PoseStack.Pose pose, VertexConsumer out, double angle, boolean glow) {
        for (int marker = 0; marker < 6; marker++) {
            double phase = marker * Math.TAU / 6 + 0.29 + angle * 0.0007;
