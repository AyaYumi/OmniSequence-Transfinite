...
final class SectionedGhostPreviewRenderer implements AutoCloseable {
    ...
    private static VertexBuffer upload(com.mojang.blaze3d.vertex.BufferBuilder.RenderedBuffer meshData) {
        if (meshData == null) {
            return null;
        }
        VertexBuffer buffer;
    ...
    private static final class ProjectionVertexConsumer implements VertexConsumer {
        ...
        @Override public void defaultColor(int r, int g, int b, int a) { delegate.defaultColor(r, g, b, Math.min(a, alpha)); }
