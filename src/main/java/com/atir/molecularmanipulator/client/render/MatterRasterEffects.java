...
public final class MatterRasterEffects {
    ...
    public static final class Animation {
        ...
        public Frame sample(float time,boolean formed,boolean running,float progress,float completion){
            ...
            previousTime=time;
            progress=Math.clamp(progress,0,1);completion=Math.clamp(completion,0,1);
    ...
    public static float idleOpacity(float time){
        ...
        double x=Math.clamp(Math.min(tick,DISPLAY_TICKS-tick)/9,0,1);return (float)(x*x*(3-2*x));
    }
    ...
    public static Vec3 mote(Frame frame,int stream,double travel){
        ...
        double t=Math.clamp(travel,0,1);return start.scale((1-t)*(1-t)).add(control.scale(2*t*(1-t))).add(end.scale(t*t));
    }
