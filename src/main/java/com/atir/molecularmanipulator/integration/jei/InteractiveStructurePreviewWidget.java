...
abstract class InteractiveStructurePreviewWidget implements IRecipeWidget, IJeiGuiEventListener {
    ...
    public void drawWidget(GuiGraphics graphics, double mouseX, double mouseY) {
        AeUiTheme.panel(graphics, 0, 0, WIDTH, height);
        AeUiTheme.insetPanel(graphics, view.left() - 1, view.top() - 1, view.right() + 1, view.bottom() + 1);
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            AeUiTheme.buttonFace(graphics, left, 4, right, 22, buttonAt(mouseX, mouseY) == i);
            text(graphics, labels[i], left + 2, 9, BUTTON_WIDTH[i] - 4, AeUiTheme.PRIMARY_TEXT, true);
        }
        // Text is emitted once, without the vanilla drop shadow, after the 3D pass.
        AeUiTheme.insetPanel(graphics, 7, 28, 99, 42);
        text(graphics, Component.literal(dimensions()),
                10, 31, 100, AeUiTheme.PRIMARY_TEXT, false);
        text(graphics, Component.translatable("gui.molecularmanipulator.jei_preview_controls"),
                6, height - 12, WIDTH - 12, AeUiTheme.PRIMARY_TEXT, true);
    }
