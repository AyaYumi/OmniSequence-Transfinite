...
public final class MatterFabricationJeiCategory implements IRecipeCategory<MatterFabricationRecipe> {
    ...
    public MatterFabricationJeiCategory(IGuiHelper helper) {
        ...
        slot = helper.createDrawable(ResourceLocation.parse("ae2:textures/guis/terminal.png"), 7, 17, 18, 18);
    }
    ...
    @Override public void draw(MatterFabricationRecipe recipe, IRecipeSlotsView slots, GuiGraphics graphics, double mouseX, double mouseY) {
        ...
        graphics.drawString(font, label, 8, 5, AeUiTheme.PRIMARY_TEXT, false);
        graphics.fill(98, 49, 117, 52, AeUiTheme.SHADOW);
        for (int i = 0; i < 4; i++) graphics.fill(115 + i, 46 + i, 116 + i, 55 - i, AeUiTheme.SHADOW);
    ...
    private static StageInfo resolveStage(MatterFabricationRecipe recipe) {
        ...
        return new StageInfo(label, List.of(label), recipe.requiresResearch() ? AeUiTheme.WARNING : AeUiTheme.MUTED_TEXT);
    }
