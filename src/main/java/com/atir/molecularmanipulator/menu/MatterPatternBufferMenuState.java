...
public record MatterPatternBufferMenuState(int page, int pages, int itemTypes, int fluidTypes,
    ...
    public MatterPatternBufferMenuState(FriendlyByteBuf buffer) {
    ...
    private static List<GenericStack> readContents(FriendlyByteBuf buffer) {
        ...
        for (int i = 0; i < size; i++) values.add(GenericStack.readBuffer(buffer));
        return values;
    }
    @Override public void writeToPacket(FriendlyByteBuf buffer) {
        ...
        for (var stack : contents) GenericStack.writeBuffer(stack, buffer);
    }
}
