...
public final class MolecularManipulatorMenu extends AEBaseMenu implements PatternSearchIndexReceiver {
    ...
    public static final MenuType<MolecularManipulatorMenu> TYPE = ForgeMenuTypeFactory.create(MolecularManipulator.id("molecular_manipulator"), MolecularManipulatorMenu::new, PatternProviderLogicHost.class);
    ...
    @GuiSync(4)
    public int occupiedPatternSlots;

    private int countedPatternRevision = Integer.MIN_VALUE;
    ...
    private void sendPatternSearchIndex() {
        if (!isClientSide() && getPlayer() instanceof ServerPlayer player
            ...
            for (var payload : PatternSearchIndexBuilder.chunks(containerId, generation, entries)) {
                com.atir.molecularmanipulator.network.PatternSearchIndexPayload.sendToPlayer(player, payload);
            }
        }
    }
    ...
    public void broadcastChanges() {
        if (isServerSide()) {
            ...
            if (countedPatternRevision != patternRevision) {
                int occupied = 0;
                for (int slot = 0; slot < patternInventory.size(); slot++) {
                    if (!patternInventory.getStackInSlot(slot).isEmpty()) occupied++;
                }
                occupiedPatternSlots = occupied;
