...
public final class ResearchCodecs {
    ...
    public static final Codec<Integer> PREREQUISITE_LEVEL = Codec.either(Codec.intRange(1, Integer.MAX_VALUE),
            Codec.STRING.comapFlatMap(value -> value.equals("max") ? DataResult.success(value)
                    : DataResult.error(() -> "Prerequisite level must be a positive integer or max"), java.util.function.Function.identity()))
