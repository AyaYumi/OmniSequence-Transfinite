...
public record ResearchVisualState(List<Task> tasks, long completionSerial, int completionStyle) {
    ...
    public record Task(long identity, int style, int round, int progress, int duration, boolean running) {
        public Task {
            ...
            progress = com.atir.molecularmanipulator.util.MathCompat.clamp(progress, 0, duration);
        }

        public float fraction(double elapsed) {
            return (float) com.atir.molecularmanipulator.util.MathCompat.clamp((progress + (running ? com.atir.molecularmanipulator.util.MathCompat.clamp(elapsed, 0, 5) : 0)) / duration, 0, 1);
        }
    }
