...
public final class MultiblockChunkLoading {
    ...
    public static void register() {
        ForgeChunkManager.setForcedChunkLoadingCallback(MolecularManipulator.MOD_ID, MultiblockChunkLoading::validateTickets);
    }
    ...
    public static void maintain(BlockEntity machine) {
        ...
        for (var chunk : required) {
            if (!existing.contains(chunk)) ForgeChunkManager.forceChunk(level, MolecularManipulator.MOD_ID, owner, chunk.x, chunk.z, true, true);
        }
        for (var chunk : existing) {
            if (!required.contains(chunk)) ForgeChunkManager.forceChunk(level, MolecularManipulator.MOD_ID, owner, chunk.x, chunk.z, false, true);
        }
    }
    ...
    public static void release(ServerLevel level, BlockPos owner) {
        ...
        for (var chunk : chunks) {
            ForgeChunkManager.forceChunk(level, MolecularManipulator.MOD_ID, owner, chunk.x, chunk.z, false, true);
        }
    }

    @SubscribeEvent
    public static void onLevelUnload(LevelEvent.Unload event) {
        // Forge saves and restores the tickets. Only drop the in-memory index.
    ...
    private static void validateTickets(ServerLevel level, TicketHelper helper) {
        for (var entry : helper.getBlockTickets().entrySet()) {
            ...
            if (!(machine instanceof MolecularCenterBlockEntity
                ...
                for (long chunk : entry.getValue().getFirst()) helper.removeTicket(entry.getKey(), chunk, false);
                var chunks = new HashSet<ChunkPos>();
                for (long chunk : entry.getValue().getSecond()) chunks.add(new ChunkPos(chunk));
