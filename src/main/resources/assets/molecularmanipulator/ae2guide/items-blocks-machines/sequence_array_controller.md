...
The array can hold 100,000 entropy and passively dissipates 25 entropy per second. If the next operation would exceed
the limit, that job pauses in **Cooling** and resumes automatically as soon as enough entropy has dissipated. Cooling
continues while the array works, which is why the bar can fill during heavy use and fall again when processing slows or
stops. Acceleration cards do not change entropy per item, but faster processing can make it accumulate more quickly.
