...
class MatterConnectedOutlineTest {
    ...
    private static int sample(String name, int mask, int x, int y) throws Exception {
        ...
        if (filler) {
            u = com.atir.molecularmanipulator.util.MathCompat.clamp(u, 2.5F / 16, 13.5F / 16);
            v = com.atir.molecularmanipulator.util.MathCompat.clamp(v, 2.5F / 16, 13.5F / 16);
        }
        return texture(filler ? "casing_top" : name).getRGB(com.atir.molecularmanipulator.util.MathCompat.clamp((int) (u * 16), 0, 15), com.atir.molecularmanipulator.util.MathCompat.clamp((int) (v * 16), 0, 15));
    }
}
