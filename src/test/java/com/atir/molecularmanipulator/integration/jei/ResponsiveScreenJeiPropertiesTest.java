...
class ResponsiveScreenJeiPropertiesTest {
    ...
    void openingBeforeInitializationDoesNotPublishInvalidDimensions() throws Exception {
        ...
        var handler = ResponsiveScreenJeiProperties.<ResponsiveContainerScreen<?>>handler();
        assertNull(handler.apply(screen));
        screen.viewport(854, 300, 332, 368);
        var properties = handler.apply(screen);
        assertNotNull(properties);
        assertEquals(854, properties.getScreenWidth());
        assertEquals(300, properties.getScreenHeight());
        assertEquals(4, properties.getGuiTop());
        assertEquals(292, properties.getGuiYSize());
    }
    ...
    void pluginRegistersTheFittedScreenBoundsForEveryMachineFamily() throws Exception {
        ...
        var properties = handler.apply(screen);
        assertEquals(109, properties.getGuiLeft());
        assertEquals(4, properties.getGuiTop());
        assertEquals(349, properties.getGuiXSize());
        assertEquals(232, properties.getGuiYSize());
        assertEquals(567, properties.getScreenWidth());
        assertEquals(240, properties.getScreenHeight());
        screen.viewport(854, 480, 430, 286);
        properties = handler.apply(screen);
        assertEquals(screen.getGuiLeft(), properties.getGuiLeft());
        assertEquals(screen.getGuiTop(), properties.getGuiTop());
        assertEquals(430, properties.getGuiXSize());
        assertEquals(286, properties.getGuiYSize());
    }
}
