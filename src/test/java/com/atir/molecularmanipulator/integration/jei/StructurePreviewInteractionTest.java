...
class StructurePreviewInteractionTest {
    ...
    void previewLeavesRoomForTheJeiHeaderAndBothMaterialRowsOnSmallScreens() {
        for (int screenHeight : new int[] {240, 270, 300, 360, 480}) {
            ...
            assertTrue(preview + StructureJeiLayout.MATERIAL_HEIGHT <= screenHeight - 84);
            assertTrue(StructureJeiLayout.materialY(13, preview) + StructureMaterialRenderer.HEIGHT
                    < preview + StructureJeiLayout.MATERIAL_HEIGHT, "Second material row and quantity must fit");
        }
    }
    ...
    void rightAndMiddleDragPanWhileWheelZoomsWithoutChangingLayer() {
        ...
        assertTrue(widget.mouseScrolled(100, 90, 2));
        assertEquals(1.3225, widget.camera.zoom, 1.0E-8);
        assertEquals(-1, widget.layer);
        assertFalse(widget.mouseScrolled(100, 200, 1));
    }
