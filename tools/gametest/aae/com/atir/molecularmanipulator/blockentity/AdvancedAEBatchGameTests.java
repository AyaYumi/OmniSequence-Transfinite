...
public final class AdvancedAEBatchGameTests {
    ...
    public static void aaeExecutesPublicBatches(GameTestHelper helper) {
        ...
        var bay = MatterFabricationStructure.worldPos(ORIGIN, Direction.NORTH,
                MatterFabricationStructure.patternAssemblyBays().getFirst());
    ...
    private static void verify(GameTestHelper helper, MatterFabricationBlockEntity controller,
        ...
        var encoded = PatternDetailsHelper.encodeProcessingPattern(List.of(new GenericStack(key, 16)), List.of(new GenericStack(output, 16)));
        ...
        var candidate = MatterRecipeIndex.get(level).candidates(MatterFabricationBatch.patternOutputs(pattern)).getFirst();
        ...
        check(helper, assembly.getBuffer().contents(false).getFirst().amount() == 1_048_576, "Matter queue owns all inputs");
        var saved = new CompoundTag();
        assembly.getBuffer().save(saved, level.registryAccess()); assembly.getBuffer().clear(); assembly.getBuffer().load(saved, level.registryAccess());
        check(helper, assembly.getBuffer().contents(false).getFirst().amount() == 1_048_576, "Accepted queue survives reload");
        ...
        check(helper, assembly.getBuffer().contents(false).getFirst().amount() == 1_048_576, "Cancellation does not duplicate machine-owned inputs");
