...
public final class MolecularCenterPatternPortGameTests {
    ...
    public static void storageBusExposesOnlySupportedMainPatterns(GameTestHelper helper) {
        ...
        var bus = cable.addPart(AEParts.STORAGE_BUS.asItem(), Direction.WEST, null);
        ...
        helper.startSequence().thenWaitUntil(() -> {
            ...
            for (var side : Direction.values()) helper.assertTrue(
                    center.getCapability(ForgeCapabilities.ITEM_HANDLER, side).orElseThrow(IllegalStateException::new) == handler,
            ...
            var processing = PatternDetailsHelper.encodeProcessingPattern(
                    new GenericStack[] {new GenericStack(AEItemKey.of(Items.STONE), 1)}, new GenericStack[] {new GenericStack(AEItemKey.of(Items.DIRT), 1)});
            ...
            var saved = center.saveWithFullMetadata();
            var restored = (MolecularCenterBlockEntity) BlockEntity.loadStatic(pos, level.getBlockState(pos), saved);
    ...
    public static void externalTransfersKeepCrystalOwnershipAcrossEmptyReloadAndDismantling(GameTestHelper helper) {
        ...
        helper.startSequence().thenWaitUntil(() -> helper.assertTrue(center.canAccessExternalPatterns() && center.isFormed(),
            ...
            var restored = BlockEntity.loadStatic(pos, level.getBlockState(pos), saved);
            ...
            var saved = reloaded.saveWithFullMetadata();
            ...
            replacement.load(BlockItem.getBlockEntityData(drop));
    ...
    private static List<ItemStack> patterns(GameTestHelper helper) {
        ...
        var planks = (CraftingRecipe) recipes.byKey(new ResourceLocation("oak_planks")).orElseThrow();
        ...
        var slab = (StonecutterRecipe) recipes.byKey(new ResourceLocation("stone_slab_from_stone_stonecutting")).orElseThrow();
        ...
        var sword = (SmithingRecipe) recipes.byKey(new ResourceLocation("netherite_sword_smithing")).orElseThrow();
